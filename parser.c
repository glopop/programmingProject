...
//used the conecpt of basic cuz we needed a base expression
...
int is_ReactBody(struct TokenList** tok, struct ParseTree** new) {
    ...
    if ((*tok)->token->type == Lpar) {
        ...
        *tok = (*tok)->next;
        ...
        // If it's not an opening parenthesis, return an error
        free_ParseTree(*new);
        return PARSING_ERROR;
    }

    while ((*tok) != NULL &&
           (*tok)->token->type != Rpar) {  // Check for closing parenthesis ')'
        count++;
        line = alloc_ParseTree();
        if (line == NULL)
            return MEMORY_ERROR;
    ...
    if ((*tok)->token->type == Rpar) {
        ...
        *tok = (*tok)->next;
        ...
        // If it's not a closing parenthesis, return an error
        free_ParseTree(*new);
        return PARSING_ERROR;
    }
...
int is_React2(struct TokenList** tok, struct ParseTree** new) {
    ...
    // Check if the current token is "react"
    if ((*tok)->token->type == react) {
        // Move to the next token
        *tok = (*tok)->next;
    } else {
        // If it's not "react", return an error
        free_ParseTree(reactToken);
        return PARSING_ERROR;
    }

    // Attach the "react" token as a child of the current ParseTree node
...
int is_PeriodicLine(struct TokenList** tok, struct ParseTree** new) {
    ...
    if (periodickey == NULL)
        return MEMORY_ERROR;
    status = is_While(tok, &periodickey);  // Assuming is_Periodic is the function for parsing "periodic" keyword
...
int is_MolecularLine(struct TokenList** tok, struct ParseTree** new) {
    ...
    if (molecularkey == NULL)
        return MEMORY_ERROR;
    status = is_For(tok, &molecularkey);  // Assuming is_Molecular is the function for parsing "molecular" keyword
    ...
    if (forcond == NULL)
        return MEMORY_ERROR;
    status = is_ReactCond(tok, &forcond);  // Assuming is_ForCond is the function for parsing the loop condition
